...
//void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer){
    ...
    //Filtering PCD - voxel grid downsampling and regionn of interest 
    //values inside Eigen::Vector4f() are x, y, z, 1
    pcl::PointCloud<pcl::PointXYZI>::Ptr filteredCloud = pointProcessorI->FilterCloud(inputCloud, 0.3f, Eigen::Vector4f(-9,-5,-2,1), Eigen::Vector4f(30,7,1,1));
    
    //Segmeting PCD - 100 iterations and distance threshold of 0.2
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = pointProcessorI->Clustering(segmentedCloud.first, 0.4, 10, 500);
    int clusterId = 0;
    std::vector<Color> colors = {Color(1,0,0), Color(1,1,0), Color(0,0,1)};
